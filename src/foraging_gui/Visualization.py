...
class PlotV(FigureCanvas):
    ...
    def _PlotBlockStructure(self):
        ...
        self.ax2.plot(
            self.B_Time,
            self.B_RewardProHistory[0][0:Len],
            color="r",
            label="p_L",
            alpha=1,
        )
        self.ax2.plot(
            self.B_Time,
            self.B_RewardProHistory[1][0:Len],
            color="b",
            label="p_R",
            alpha=1,
        )
    ...
    def _PlotLicks(self):
        if self.B_CurrentTrialN < 1:
            return
        self.ax1.plot(
            self.B_LeftLickTime - self.B_TrialStartTime[0],
            np.zeros(len(self.B_LeftLickTime)) - 0.4,
            "k|",
        )
        self.ax1.plot(
            self.B_RightLickTime - self.B_TrialStartTime[0],
            np.zeros(len(self.B_RightLickTime)) + 1.4,
            "k|",
        )
        self.draw()
                    ...
                    if current_date in all_dates:
                        if current_valve == "Left":
                            line = self.ax1.plot(
                                ...
                                sorted_Y,
                                "o-",
                                label=current_date + "_left valve",
                            )
                            ...
                            line = self.ax1.plot(
                                ...
                                sorted_Y,
                                "o-",
                                label=current_date + "_right valve",
                            )
                        # fit the curve
...
class PlotLickDistribution(FigureCanvas):
    ...
    def _Update(self, GeneratedTrials=None):
        ...
        self.ax1.hist(
            np.diff(GeneratedTrials.B_LeftLickTime),
            bins=custom_x_values,
            color="red",
            alpha=0.7,
            label="Left licks",
        )
        self.ax2.hist(
            np.diff(GeneratedTrials.B_RightLickTime),
            bins=custom_x_values,
            color="blue",
            alpha=0.7,
            label="Right licks",
...
class PlotTimeDistribution(FigureCanvas):
    ...
    def _Update(self, win):
        ...
        self.ax1.plot(
            [np.nanmean(SampledBlockLen), np.nanmean(SampledBlockLen)],
            [0, np.max(Re1[0])],
            label="Average",
        )
        self.ax2.plot(
