...
class PlotV(FigureCanvas):
    ...
    def _PlotChoice(self):
        ...
        if self.B_BaitHistory.shape[1] > self.B_AnimalResponseHistory.shape[0]:
            ...
            self.ax2.eventplot(
                NewTrialStart.reshape((1,)), lineoffsets=0.5, linelengths=2, linewidth=2, color="k", alpha=0.3
            )
            if self.B_BaitHistory[0][-1] == True:
                self.ax1.plot(
                    NewTrialStart2, -0.2, "kD", label="Reward available", markersize=self.MarkerSize, alpha=0.4
                )
        ...
        if np.size(self.B_AutoLeftWaterStartTime) != 0:
            self.ax1.plot(
                ...
                np.zeros(len(self.B_AutoLeftWaterStartTime)) + 0.4,
                "bo",
                markerfacecolor=(0, 1, 0, 1),
                markersize=self.MarkerSize,
            )
        if np.size(self.B_AutoRightWaterStartTime) != 0:
            self.ax1.plot(
                self.B_AutoRightWaterStartTime,
                np.zeros(len(self.B_AutoRightWaterStartTime)) + 0.6,
                "bo",
                markerfacecolor=(0, 1, 0, 1),
                markersize=self.MarkerSize,
                label="AutoWater",
        ...
        if np.size(self.B_ManualLeftWaterStartTime) != 0:
            self.ax1.plot(
                ...
                np.zeros(len(self.B_ManualLeftWaterStartTime)) + 0.3,
                "bs",
                markerfacecolor=(0, 1, 0, 1),
                markersize=self.MarkerSize,
            )
        if np.size(self.B_ManualRightWaterStartTime) != 0:
            self.ax1.plot(
                ...
                np.zeros(len(self.B_ManualRightWaterStartTime)) + 0.7,
                "bs",
                markerfacecolor=(0, 1, 0, 1),
                markersize=self.MarkerSize,
                label="ManualWater",
        ...
        for condition in condition_list:
            ...
            self.ax1.plot(
                self.B_BTime[Optogenetics_On],
                np.zeros(len(self.B_BTime[Optogenetics_On])) + 1.5,
                "o",
                markeredgecolor=current_color,
                markerfacecolor=current_color,
                label="Opto condition " + str(condition),
                markersize=self.MarkerSize,
                alpha=1,
            )
        ...
        if np.size(LeftBait) != 0:
            self.ax1.plot(
                self.B_BTime[LeftBait],
                np.zeros(len(self.B_BTime[LeftBait])) - 0.2,
                "kD",
                markersize=self.MarkerSize,
                alpha=0.2,
            )
        if np.size(RightBait) != 0:
            self.ax1.plot(
                self.B_BTime[RightBait],
                np.zeros(len(self.B_BTime[RightBait])) + 1.2,
                "kD",
                markersize=self.MarkerSize,
                alpha=0.2,
            )
        if np.size(LeftChoice) != 0:
            self.ax1.plot(
                self.B_Time[LeftChoice],
                np.zeros(len(self.B_Time[LeftChoice])) + 0,
                "go",
        ...
        if np.size(LeftChoice_Rewarded) != 0:
            self.ax1.plot(
                self.B_Time[LeftChoice_Rewarded],
                np.zeros(len(self.B_Time[LeftChoice_Rewarded])) + 0.2,
                "go",
                markerfacecolor=(0, 1, 0, 1),
                label="Rewarded",
                markersize=self.MarkerSize,
        ...
        if np.size(RightChoice) != 0:
            self.ax1.plot(
                self.B_Time[RightChoice],
                np.zeros(len(self.B_Time[RightChoice])) + 1,
                "go",
                markerfacecolor=(1, 1, 1, 1),
                markersize=self.MarkerSize,
            )
        if np.size(RightChoice_Rewarded) != 0:
            self.ax1.plot(
                self.B_Time[RightChoice_Rewarded],
                np.zeros(len(self.B_Time[RightChoice_Rewarded])) + 0.8,
                "go",
                markerfacecolor=(0, 1, 0, 1),
                markersize=self.MarkerSize,
            )
        if np.size(NoResponse) != 0:
            self.ax1.plot(
                self.B_Time[NoResponse],
                np.zeros(len(self.B_Time[NoResponse])) + 0.5,
                "Xk",
                label="NoResponse",
                markersize=self.MarkerSize,
                alpha=0.2,
        ...
        if self.B_CurrentTrialN > self.kernel_size:
            self.ax2.plot(
                self.B_Time[self.kernel_size - 1 :],
                ResponseHistoryF[: -self.kernel_size + 1],
                "k",
                label="Choice_frac",
                linewidth=2,
                alpha=0.8,
            ...
            self.ax2.plot(
                self.B_Time[self.kernel_size - 1 :],
                RewardedHistoryF[: -self.kernel_size + 1],
                "g",
                label="reward_frac",
                linewidth=1,
                alpha=0.8,
            ...
            self.ax2.plot(
                self.B_Time[self.kernel_size - 1 :],
                SuccessHistoryF[: -self.kernel_size + 1],
                "c",
                label="finish_frac",
                alpha=0.2,
            )
...
class PlotWaterCalibration(FigureCanvas):
    def __init__(self, water_win, dpi=100, width=5, height=4):
        self.fig = Figure(figsize=(width, height), dpi=dpi)
        gs = GridSpec(10, 30, wspace=3, hspace=0.1, bottom=0.1, top=0.9, left=0.08, right=0.98)
...
class PlotLickDistribution(FigureCanvas):
    ...
    def _Update(self, GeneratedTrials=None):
        ...
        self.ax1.hist(
            np.diff(GeneratedTrials.B_LeftLickTime), bins=custom_x_values, color="red", alpha=0.7, label="Left licks"
        )
        self.ax2.hist(
            np.diff(GeneratedTrials.B_RightLickTime), bins=custom_x_values, color="blue", alpha=0.7, label="Right licks"
        )
