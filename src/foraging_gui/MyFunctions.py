...
class GenerateTrials:
    ...
    def _GenerateATrial(self):
        ...
        if self.task_logic.task_parameters.uncoupled_reward is None:
            ...
            if self.B_CurrentTrialN == -1 or not hasattr(
                ...
                self.uncoupled_blocks.add_choice(["L", "R", "ignored"][int(self.B_AnimalResponseHistory[-1])])
                _, msg_uncoupled_block = self.uncoupled_blocks.next_trial()
            ...
            for i, side in enumerate(["L", "R"]):
                ...
                self.BlockLenHistory[i] = list(np.diff([0] + self.uncoupled_blocks.block_ends[side]))
        ...
        self.B_RewardProHistory = np.append(
            self.B_RewardProHistory, self.B_CurrentRewardProb.reshape(self.B_LickPortN, 1), axis=1
        )
    ...
    def _CheckSessionControl(self):
        ...
        if self.opto_model.session_control is not None:
            session_control_block_length = (
                self.operation_control_model.auto_stop.max_trial * self.opto_model.session_control.session_fraction
            )
    ...
    def _get_warmup_state(self):
        ...
        if (
            ...
            and finish_ratio >= self.task_logic.task_parameters.warmup.min_finish_ratio
            and abs(choice_ratio - 0.5) <= self.task_logic.task_parameters.warmup.max_choice_ratio_bias
        ):
    ...
    def _check_coupled_block_transition(self):
        ...
        if np.all(self.B_ANewBlock == 1) and self.AllRewardThisBlock != -1:
            if (
                self.AllRewardThisBlock < self.task_logic.task_parameters.block_parameters.min_reward
        ...
        for i in range(len(self.B_RewardProHistory)):
            if np.all(self.B_RewardProHistory[i] == self.B_CurrentRewardProb[i]):
                ...
                self.CurrentBlockLen.append(
                    ...
                    - 1
                    - np.max(np.where(self.B_RewardProHistory[i] != self.B_CurrentRewardProb[i]))
                )
            ...
            if (
                ...
                for i in range(len(self.B_RewardedHistory)):
                    B_RewardedHistory[i] = np.logical_and(
                        self.B_AnimalResponseHistory[Ind] == i, self.B_BaitHistory[i][Ind]
                    )
        # get the block length and index of the current trial
        for i in range(B_RewardProHistory.shape[0]):
            length, indexN = self._consecutive_length(B_RewardProHistory[i], B_RewardProHistory[i][-1])
    ...
    def _GetBasic(self):
        ...
        BS_auto_water_right, BS_earned_reward_right, BS_AutoWater_N_right, BS_EarnedReward_N_right = (
        ...
        if Len > 0:
            ...
            if self.session_model.experiment in ["Coupled Baiting", "Uncoupled Baiting"]:
                ...
                self.B_for_eff_optimal, self.B_for_eff_optimal_random_seed = self.foraging_eff_no_baiting(
                    reward_rate, p_Ls, p_Rs, random_number_L, random_number_R
                )
            else:
    ...
    def foraging_eff(
        ...
        for b_start, b_end in zip(block_trans[:-1], block_trans[1:]):
            ...
            reward_refill = np.vstack(
                [reward_refills[1 - side_max][b_start:b_end], reward_refills[side_max][b_start:b_end]]
        ...
        if len(right) > 0 and len(left) > 0:
            ...
            if same_side_frac >= threshold:
                self.win.same_side_lick_interval.setText(
                    f"Percentage of same side lick intervals under 100 ms is over 10%: {same_side_frac * 100:.2f}%."
                )
                logging.warning(
            ...
            merged_sorted = np.array(sorted(np.concatenate((stacked_right, stacked_left)), key=lambda x: x[1]))
            ...
            if cross_side_frac >= threshold:
                self.win.cross_side_lick_interval.setText(
                    f"Percentage of cross side lick intervals under 100 ms is over 10%: {cross_side_frac * 100:.2f}%."
                )
                logging.warning(
    ...
    def _ShowInformation(self):
        ...
        if self.win.default_ui == "ForagingGUI.ui":
            ...
            if hasattr(self.win, "B_Bias_R"):
                ...
                self.win.info_performance_essential_2 += f"Bias: {self.win.B_Bias_R:.2f} ({bias_side})"
            else:
    ...
    def _CheckStop(self):
        ...
        if (
            ...
            and len(np.where(non_auto_reward[-oc.auto_stop.ignore_win :] == 2)[0]) >= stop_ignore
        ):
            stop = True
            ...
            warning_label_text = (
                "Stop because ignore trials exceed or equal: " + f"{threshold}% of {oc.auto_stop.ignore_win}"
            )
    ...
    def _CheckAutoWater(self):
        ...
        if self.task_logic.task_parameters.auto_water is not None:
            ...
            if UnrewardedN <= 0:
                ...
                logging.warning(
                    f"Auto water because ignored trials exceed: {IgnoredN}", extra={"tags": [self.win.warning_log_tag]}
                )
                self.CurrentAutoReward = 1
            else:
                if (
    ...
    def _GetLaserWaveForm(self):
        ...
        if laser.end is not None and laser.start is not None:
            if laser.start.interval_condition == "Trial start" and laser.end.interval_condition == "Go cue":
                ...
                laser.duration = (
                    self.task_logic.task_parameters.response_time.response_time - laser.start.offset + laser.end.offset
                )
    ...
    def _ProduceWaveForm(self, Amplitude):
        ...
        if self.selected_condition.protocol.name == "Sine":
            resolution = (
                self.opto_model.sample_frequency * self.selected_condition.duration
            )  # how many datapoints to generate
            cycles = (
                self.selected_condition.duration * self.selected_condition.protocol.frequency
            )  # how many sine cycles
            length = np.pi * 2 * cycles
            self.my_wave = (
                Amplitude * (1 + np.sin(np.arange(0 + 1.5 * math.pi, length + 1.5 * math.pi, length / resolution))) / 2
            )
            # add ramping down
            ...
            if PulseIntervalPoints < 0:
                logging.warning(
                    "Pulse frequency and pulse duration are not compatible!", extra={"tags": [self.win.warning_log_tag]}
                )
        ...
        if hasattr(self.selected_condition.protocol, "ramp_down"):
            ...
            if ramp_down > 0:
                if ramp_down > self.selected_condition.duration:
                    logging.warning(
                        "Ramping down is longer than the laser duration!", extra={"tags": [self.win.warning_log_tag]}
                    )
                else:
                    Constant = np.ones(
    ...
    def _InitiateATrial(self, Channel1, Channel4):
        ...
        self.B_AutoWaterTrial = np.append(
            self.B_AutoWaterTrial, np.array(self.CurrentAutoRewardTrial).reshape(2, 1), axis=1
        )
        ...
        if self.CurrentSimulation == False:  # run simulation if it's true
            ...
            if self.B_LaserOnTrial[self.B_CurrentTrialN] == 1:
                if self.selected_condition.start.interval_condition is not None:
                    ...
                    logging.warning(
                        "Unindentified optogenetics start event!", extra={"tags": [self.win.warning_log_tag]}
                    )
            ...
            for comport in list_comports():
                if comport.vid == VID_NEWSCALE:
                    if comport.pid == PID_NEWSCALE:
                        ...
                        instances.append(cls(serial_number, pyserial_device=Serial(comport.device)))  # does this work?
        elif PLATFORM == "win32":
...
class EphysRecording:
    ...
    def start_open_ephys_recording(self):
        ...
        """
        r1 = requests.put(self.api_endpoint + "recording", json={"prepend_text": self.mouse_id + "_"})

        r2 = requests.put(self.api_endpoint + "status", json={"mode": "RECORD"})
    ...
    def stop_open_ephys_recording(self):
        ...
        r = requests.put(self.api_endpoint + "status", json={"mode": "ACQUIRE"})
