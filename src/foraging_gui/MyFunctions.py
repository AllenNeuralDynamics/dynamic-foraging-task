...
class GenerateTrials():
        ...
        if self.TP_Task in ['Coupled Baiting','Coupled Without Baiting','RewardN']:
            ...
            if self.B_CurrentTrialN == -1 or \
                ...
                self.uncoupled_blocks = UncoupledBlocks(                 
                    ...
                    block_min=self.task_logic.task_parameters.block_min,
                    block_max=self.task_logic.task_parameters.block_max,
    ...
    def _CheckSessionControl(self):
        ...
        if self.TP_SessionWideControl=='on':
            session_control_block_length=self.task_logic.task_parameters.max_trial*float(self.TP_FractionOfSession)
            if self.TP_SessionStartWith=='on':
                initial_state=1
            elif self.TP_SessionStartWith=='off':
                initial_state=0
            calculated_state=np.zeros(self.task_logic.task_parameters.max_trial)
            numbers=np.arange(self.task_logic.task_parameters.max_trial)
    ...
    def _get_uncoupled_reward_prob_pool(self):
        ...
        input_string = self.task_logic.task_parameters.uncoupled_reward
    ...
    def _CheckWarmUp(self):
        ...
        if self.task_logic.task_parameters.warmup== 'off':
            return
        warmup=self._get_warmup_state()
        if warmup==0 and self.task_logic.task_parameters.warmup == 'on':
            # set warm up to off
    ...
    def _get_warmup_state(self):
        ...
        warm_windowsize=self.task_logic.task_parameters.warm_windowsize
        ...
        if finish_trial>=self.task_logic.task_parameters.warm_min_trial and \
                finish_ratio>=self.task_logic.task_parameters.warm_min_finish_ratio\
                and abs(choice_ratio-0.5)<=self.task_logic.task_parameters.warm_max_choice_ratio_bias:
    ...
    def _generate_next_coupled_block(self):
        ...
        self.RewardPairs=self.B_RewardFamilies[tp.reward_family-1][:tp.reward_pairs_n]
        self.RewardProb=np.array(self.RewardPairs)/np.expand_dims(np.sum(self.RewardPairs,axis=1),axis=1)*tp.base_reward_sum
        ...
        if tp.randomness=='Exponential':
            self.BlockLen = np.array(int(np.random.exponential(tp.block_beta,1)+tp.block_min))
        elif tp.randomness=='Even':
            self.BlockLen=  np.array(np.random.randint(tp.block_min, tp.block_max+1))
        if self.BlockLen>tp.block_max:
            self.BlockLen=tp.block_max
    ...
    def _check_coupled_block_transition(self):
        ...
        if np.all(self.B_ANewBlock==1) and self.AllRewardThisBlock!=-1:
            if self.AllRewardThisBlock < self.task_logic.task_parameters.block_min_reward \
    ...
    def _check_advanced_block_switch(self):
        ...
        if self.task_logic.task_parameters.advanced_block_auto.value == 'off':
        ...
        if self.B_CurrentTrialN>kernel_size:
            ...
            Delta=abs((self.B_CurrentRewardProb[0]-self.B_CurrentRewardProb[1])*self.task_logic.task_parameters.switch_thr)
            ...
            if consecutive_lengths.size==0:
                ...
                return
            if self.task_logic.task_parameters.advanced_block_auto.value == 'now':
                # the courrent condition is qualified
                if len(OkPoints) in consecutive_indices[consecutive_lengths>self.task_logic.task_parameters.points_in_a_row][:,1]+1:
                    self.AdvancedBlockSwitchPermitted=1
                else:
                    self.AdvancedBlockSwitchPermitted=0
            elif self.task_logic.task_parameters.advanced_block_auto.value == 'once':
                # it happens before
                if np.any(consecutive_lengths>self.task_logic.task_parameters.points_in_a_row):
                    self.AdvancedBlockSwitchPermitted=1
                else:
                    self.AdvancedBlockSwitchPermitted=0
            else:
    ...
    def _ShowInformation(self):
        ...
        if self.win.default_ui=='ForagingGUI.ui':
            ...
            if (self.task_logic.task_parameters.auto_reward or
                self.task_logic.task_parameters.block_min_reward>0) and self.win.Start.isChecked():
                # show the next trial
    ...
    def _CheckStop(self):
        ...
        tp = self.task_logic.task_parameters
        stop_ignore=round(tp.auto_stop_ignore_win*tp.auto_stop_ignore_ratio_threshold)
        max_trial=tp.max_trial-2 # trial number starts from 0
        max_time=tp.max_time*60 # convert minutes to seconds
        ...
        if self.BS_CurrentRunningTime/60 >= min_time and len(np.where(non_auto_reward[-tp.auto_stop_ignore_win:] == 2)[0]) >= stop_ignore:
            stop=True
            threshold = tp.auto_stop_ignore_ratio_threshold*100
            msg = f'Stopping the session because the mouse has ignored at least ' \
                  f'{threshold}% of {tp.auto_stop_ignore_win} ' \
            ...
            stop=True
            msg = 'Stopping the session because the session running time has reached {} minutes'.format(max_time)
    ...
    def _CheckAutoWater(self):
        ...
        if self.task_logic.task_parameters.auto_reward:
            unrewarded = self.task_logic.task_parameters.unrewarded
            ignored=self.task_logic.task_parameters.ignored
            if unrewarded<=0:
                self.CurrentAutoReward=1
                logging.warning(f'Auto water because unrewarded trials exceed: {unrewarded}',
                                extra={'tags': [self.win.warning_log_tag]})
            elif  ignored <=0:
                logging.warning(f'Auto water because ignored trials exceed: {ignored}',
                                extra={'tags': [self.win.warning_log_tag]})
                self.CurrentAutoReward=1
            else:
                if np.shape(self.B_AnimalResponseHistory)[0]>=ignored or \
                        np.shape(self.B_RewardedHistory[0])[0]>=unrewarded:
                    # auto reward is considered as reward
                    B_RewardedHistory=self.B_RewardedHistory.copy()
                    Ind=range(len(self.B_RewardedHistory[0]))
                    for i in range(len(self.B_RewardedHistory)):
                        B_RewardedHistory[i]=np.logical_or(self.B_RewardedHistory[i],self.B_AutoWaterTrial[i][Ind])
                    if np.all(self.B_AnimalResponseHistory[-ignored:]==2) and np.shape(self.B_AnimalResponseHistory)[0]>=ignored:
                        self.CurrentAutoReward=1
                        logging.warning(f'Auto water because ignored trials exceed: {ignored}',
                                        extra={'tags': [self.win.warning_log_tag]})
                    elif (np.all(B_RewardedHistory[0][-unrewarded:]==False)
                          and np.all(B_RewardedHistory[1][-unrewarded:]==False)
                          and np.shape(B_RewardedHistory[0])[0]>=unrewarded):
                        self.CurrentAutoReward=1
                        logging.warning(f'Auto water because unrewarded trials exceed: {unrewarded}',
                                        extra={'tags': [self.win.warning_log_tag]})
                    else:
    ...
    def _GetLaserWaveForm(self):
        ...
        if (self.CLP_LaserStart=='Trial start' or self.CLP_LaserStart=='Go cue' or self.CLP_LaserStart=='Reward outcome') and self.CLP_LaserEnd=='NA':
            ...
            self.CLP_CurrentDuration=self.task_logic.task_parameters.response_time-self.CLP_OffsetStart+self.CLP_OffsetEnd
        else:
            pass
    ...
    def _InitiateATrial(self,Channel1,Channel4):
        ...
        if self.CurrentAutoReward==1:
            ...
            if self.task_logic.task_parameters.auto_water_type.value == 'Natural':
                for i in range(len(self.CurrentBait)):
                    if self.CurrentBait[i]==True:
                        self.CurrentAutoRewardTrial[i]=1
            if self.task_logic.task_parameters.auto_water_type.value == 'Both':
                self.CurrentAutoRewardTrial=[1,1]
            if self.task_logic.task_parameters.auto_water_type.value == 'High pro':
                if self.B_CurrentRewardProb[0]>self.B_CurrentRewardProb[1]:
        ...
        if self.CurrentSimulation==False: # run simulation if it's true
            ...
            Channel1.RewardConsumeTime(self.task_logic.task_parameters.reward_consume_time)
    ...
    def _GetAnimalResponse(self,Channel1,Channel3,Channel4):
        ...
        multiplier = self.task_logic.task_parameters.multiplier
