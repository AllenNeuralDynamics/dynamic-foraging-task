...
def bonsai_to_nwb(fname, save_folder=save_folder):
    ...
    nwbfile.add_scratch(df_metadata, name="metadata", description="Some important session-wise meta data")
    ...
    nwbfile.add_trial_column(
        name="base_reward_probability_sum", description=f"The summation of left and right reward probability"
    )
    ...
    # block
    nwbfile.add_trial_column(
        name="block_beta", description=f"The beta of exponential distribution to generate the block length"
    )
    ...
    # delay duration
    nwbfile.add_trial_column(
        name="delay_beta", description=f"The beta of exponential distribution to generate the delay duration(s)"
    )
    ...
    nwbfile.add_trial_column(
        name="delay_duration", description=f"The expected time duration between delay start and go cue start"
    )
    # ITI duration
    nwbfile.add_trial_column(
        name="ITI_beta", description=f"The beta of exponential distribution to generate the ITI duration(s)"
    )
    ...
    nwbfile.add_trial_column(
        name="ITI_duration", description=f"The expected time duration between trial start and ITI start"
    )
    # response duration
    nwbfile.add_trial_column(
    ...
    nwbfile.add_trial_column(
        name="reward_consumption_duration", description=f"The duration for the animal to consume the reward"
    )
    # reward delay
    ...
    nwbfile.add_trial_column(
        name="laser_start_offset", description=f"Laser start is aligned to an event with an offset"
    )
    ...
    nwbfile.add_trial_column(
        name="minimum_opto_interval", description=f"Minimum interval between two optogenetics trials (number of trials)"
    )
    ...
    nwbfile.add_trial_column(
        name="auto_train_curriculum_version", description=f"The version of the auto training curriculum"
    )
    nwbfile.add_trial_column(
        name="auto_train_curriculum_schema_version", description=f"The schema version of the auto training curriculum"
    )
    nwbfile.add_trial_column(name="auto_train_stage", description=f"The current stage of auto training")
    nwbfile.add_trial_column(
        name="auto_train_stage_overridden", description=f"Whether the auto training stage is overridden"
    )

    # determine lickspout keys based on stage position keys
    stage_positions = getattr(obj, "B_StagePositions", [{}])
    nwbfile.add_trial_column(
        name="lickspout_position_x", description=f"x position (um) of the lickspout position (left-right)"
    )
    nwbfile.add_trial_column(
        name="lickspout_position_z", description=f"z position (um) of the lickspout position (up-down)"
    )
    if len(stage_positions) > 0 and list(stage_positions[0].keys()) == ["x", "y1", "y2", "z"]:  # aind stage
        ...
        nwbfile.add_trial_column(
            name="lickspout_position_y", description=f"y position (um) of the lickspout position (forward-backward)"
        )
    # add reward size
    ...
    for i in range(len(obj.B_TrialEndTime)):
        ...
        if Sc == 0:
            ...
            Laser1Power = float(
                eval(
                    _get_field(
                        obj,
                        field_list=[f"TP_Laser1_power_{Sc}", f"TP_LaserPowerLeft_{Sc}"],
                        index=i,
            ...
            Laser2Power = float(
                eval(
                    _get_field(
                        obj,
                        field_list=[f"TP_Laser2_power_{Sc}", f"TP_LaserPowerRight_{Sc}"],
                        index=i,
                        default="[np.nan,np.nan]",
                    )
                )[1]
        ...
        trial_kwargs = {
            ...
            "reward_random_number_left": _get_field(
                obj, "B_CurrentRewardProbRandomNumber", index=i, default=[np.nan] * 2
            )[0],
            "reward_random_number_right": _get_field(
                obj, "B_CurrentRewardProbRandomNumber", index=i, default=[np.nan] * 2
            )[1],
            ...
            else obj.B_AutoWaterTrial[i],  # Back-compatible with old autowater format
            "auto_waterR": obj.B_AutoWaterTrial[1][i]
            if type(obj.B_AutoWaterTrial[0]) is list
            ...
            "auto_train_curriculum_version": _get_field(
                obj, "TP_auto_train_curriculum_version", index=i, default="None"
            ),
            "auto_train_curriculum_schema_version": _get_field(
                obj, "TP_auto_train_curriculum_schema_version", index=i, default="None"
            ),
