...
class LoadedMouseSlimsHandler:
    ...
    def load_mouse_curriculum(
        ...
        try:
            ...
            self.curriculum, self.trainer_state, self.metrics, attachments, self._loaded_slims_session = (
            ...
            oc = OperationalControl(**self.slims_client.fetch_attachment_content(oc_att).json())
            ...
            if FiberPhotometry.__name__ in attachment_names:
                ...
                fip_model.enabled = stage_list.index(self.trainer_state.stage.name) >= stage_list.index(
                    fip_model.stage_start
                )
            else:
                fip_model = None
    ...
    def write_loaded_mouse(
        ...
        if self.metrics is not None and self.slims_client is not None:  # loaded mouse
            ...
            self.slims_client.add_attachment_content(
                record=slims_model, name=AindBehaviorSessionModel.__name__, content=session_model.model_dump_json()
            )

            # add operational control model
            self.slims_client.add_attachment_content(
                record=slims_model, name=operation_control_model.name, content=operation_control_model.model_dump_json()
            )

            # add opto model
            if opto_model.laser_colors != []:
                self.slims_client.add_attachment_content(
                    record=slims_model, name=opto_model.name, content=opto_model.model_dump_json()
                )

            # Add fip model
            stage_list = get_args(STAGE_STARTS)
            if fip_model.enabled or stage_list.index(self.trainer_state.stage.name) < stage_list.index(
                fip_model.stage_start
            ):
                self.slims_client.add_attachment_content(
                    record=slims_model, name=fip_model.name, content=fip_model.model_dump_json()
                )
    ...
    def update_loaded_session_attachments(self, attachment_name: str, serialized_json: str) -> None:
        """
        ...
        if self.slims_client is not None:
            ...
            self.slims_client.add_attachment_content(record=fetched, name=attachment_name, content=serialized_json)
    ...
    def get_loaded_mouse_offset(self) -> StageCoords or None:
        ...
        if self._loaded_mouse_id is not None and self.slims_client is not None:
            ...
            return {"x": mouse.x_offset, "y": mouse.y_offset, "z": mouse.z_offset}
        else:
            self.log.info("No mouse loaded so can't return offset.")

    def set_loaded_mouse_offset(self, x: float = None, y: float = None, z: float = None) -> None:
        """
