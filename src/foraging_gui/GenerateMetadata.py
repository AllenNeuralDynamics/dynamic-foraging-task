...
class generate_metadata:
    ...
    def _get_funding_source(self):
        ...
        self.funding_source = [
            Funding(
                funder=getattr(
                    Organization,
                    self.name_mapper["institute"][self.Obj["meta_data_dialog"]["session_metadata"]["FundingSource"]],
                ),
    ...
    def _get_session_time(self):
        ...
        all_streams = [self.behavior_streams, self.high_speed_camera_streams, self.ephys_streams, self.ophys_streams]
    ...
    def _handle_edge_cases(self):
        ...
        self._initialize_fields(
            dic=self.Obj["Camera_dialog"], keys=["camera_start_time", "camera_stop_time"], default_value=""
        )
        ...
        if (
            ...
            or (self.Obj["meta_data_dialog"]["session_metadata"]["LickSpoutReferenceArea"] == "")
            or (self.Obj["meta_data_dialog"]["session_metadata"]["LickSpoutReferenceX"] == "")
            or (self.Obj["meta_data_dialog"]["session_metadata"]["LickSpoutReferenceY"] == "")
            or (self.Obj["meta_data_dialog"]["session_metadata"]["LickSpoutReferenceZ"] == "")
        ):
        ...
        self._initialize_fields(
            dic=self.Obj, keys=["fiber_photometry_start_time", "fiber_photometry_end_time"], default_value=""
        )
        ...
        self._initialize_fields(
            dic=self.Obj["meta_data_dialog"]["session_metadata"], keys=["ProjectName"], default_value=""
        )
        ...
        if "PtotocolID" in self.Obj["meta_data_dialog"]["session_metadata"]:
            self.Obj["meta_data_dialog"]["session_metadata"]["ProtocolID"] = self.Obj["meta_data_dialog"][
                "session_metadata"
            ]["PtotocolID"]
    ...
    def _combine_data_streams(self):
        ...
        if self.data_streams == []:
            ...
            self.data_streams[0].stream_modalities = (
                self.data_streams[0].stream_modalities + self.ophys_streams[0].stream_modalities
            )
            self.data_streams[0].stream_start_time = min(
                self.data_streams[0].stream_start_time, self.ophys_streams[0].stream_start_time
            )
            self.data_streams[0].stream_end_time = max(
                self.data_streams[0].stream_end_time, self.ophys_streams[0].stream_end_time
            )
            ...
            self.data_streams[0].light_sources = (
                self.data_streams[0].light_sources + self.ophys_streams[0].light_sources
            )
            ...
            self.data_streams[0].fiber_connections = (
                self.data_streams[0].fiber_connections + self.ophys_streams[0].fiber_connections
            )
        ...
        if self.data_streams == []:
            ...
            self.data_streams[0].stream_modalities = (
                self.data_streams[0].stream_modalities + self.ephys_streams[0].stream_modalities
            )
            self.data_streams[0].stream_start_time = min(
                self.data_streams[0].stream_start_time, self.ephys_streams[0].stream_start_time
            )
            self.data_streams[0].stream_end_time = max(
                self.data_streams[0].stream_end_time, self.ephys_streams[0].stream_end_time
            )
            ...
            self.data_streams[0].ephys_modules = (
                self.data_streams[0].ephys_modules + self.ephys_streams[0].ephys_modules
            )
            self.data_streams[0].stick_microscopes = (
                self.data_streams[0].stick_microscopes + self.ephys_streams[0].stick_microscopes
            )
        ...
        if len(self.ephys_streams) > 2:
            ...
            for ephys_stream in self.ephys_streams[2:]:
                self.data_streams2.stream_start_time = min(
                    self.data_streams2.stream_start_time, ephys_stream.stream_start_time
                )
                self.data_streams2.stream_end_time = max(
                    self.data_streams2.stream_end_time, ephys_stream.stream_end_time
                )
        ...
        if self.Obj["Camera_dialog"]["camera_start_time"] != "" and self.camera_names != []:
            if self.Obj["Camera_dialog"]["camera_stop_time"] == "":
                ...
                camera_stop_time = datetime.strptime(
                    self.Obj["Camera_dialog"]["camera_stop_time"], "%Y-%m-%d %H:%M:%S.%f"
                )
            self.high_speed_camera_streams.append(
                Stream(
                    ...
                    stream_start_time=datetime.strptime(
                        self.Obj["Camera_dialog"]["camera_start_time"], "%Y-%m-%d %H:%M:%S.%f"
                    ),
    ...
    def _get_photometry_detectors(self):
        ...
        ) - datetime.strptime(self.Obj["fiber_photometry_start_time"], "%Y-%m-%d %H:%M:%S.%f")
    ...
    def _get_output_parameters(self):
        ...
        water_after_session = float(_get_field(self.Obj, field_list=["ExtraWater", "SuggestedWater"], default=np.nan))
    ...
    def _get_reward_probability(self):
        ...
        if self.session_model.experiment in ["Uncoupled Baiting", "Uncoupled Without Baiting"]:
            ...
            RewardPairs = self.Obj["B_RewardFamilies"][tp.reward_probability.family - 1][
        ...
        for ind_probe, probe in enumerate(self.probe_names):
            if probe in self.Obj["meta_data_dialog"]["session_metadata"]["probes"]:
                self.ephys_modules.append(
                    ManipulatorModule(
                        rotation_angle=self.Obj["meta_data_dialog"]["session_metadata"]["probes"][probe][
                            "RotationAngle"
                        ],
            ...
            self.opto_calibration.append(
                Calibration(
                    calibration_date=datetime.strptime(
                        current_calibration["latest_calibration_date"], "%Y-%m-%d"
                    ).date(),
                        ...
                        if Protocol == "Sine":
                            for Frequency in RecentLaserCalibration[color][Protocol]:
                                ...
                                if f"Laser_{laser_tag}" in RecentLaserCalibration[color][Protocol][Frequency]:
                                    current_calibration = RecentLaserCalibration[color][Protocol][Frequency][
                                        f"Laser_{laser_tag}"
                                    ]["LaserPowerVoltage"]
                                else:
                                    if laser_tag == 1:
    ...
    def _parse_water_calibration(self):
        ...
        for side in sides:
            ...
            sorted_X, sorted_Y = PlotWaterCalibration._GetWaterCalibration(
                self, self.WaterCalibrationResults, self.RecentWaterCalibrationDate, side
            )
    ...
    def _get_reward_delivery(self):
        ...
        if "B_StagePositions" in self.Obj.keys():
            ...
            reference_spout_position = [
                float(self.Obj["meta_data_dialog"]["session_metadata"]["LickSpoutReferenceX"]),
                float(self.Obj["meta_data_dialog"]["session_metadata"].get("LickSpoutReferenceY", None)),
                float(self.Obj["meta_data_dialog"]["session_metadata"].get("LickSpoutReferenceY1", None)),
                float(self.Obj["meta_data_dialog"]["session_metadata"].get("LickSpoutReferenceY2", None)),
