...
from aind_data_schema.components.coordinates import RelativePosition, Translation3dTransform, Rotation3dTransform, Axis, \
    AxisName
...
class generate_metadata:
    ...
    def _mapper(self):
        ...
        self.name_mapper = {
            ...
            'camera_list': ['SideCameraLeft', 'SideCameraRight', 'BottomCamera', 'BodyCamera'],
    ...
    def _get_funding_source(self):
        ...
        self.funding_source = [Funding(
            funder=getattr(Organization, self.name_mapper['institute'][
                self.Obj['meta_data_dialog']['session_metadata']['FundingSource']]),
    ...
    def _handle_edge_cases(self):
        ...
        self._initialize_fields(dic=self.Obj['Camera_dialog'], keys=['camera_start_time', 'camera_stop_time'],
                                default_value='')
        ...
        if ('B_StagePositions' not in self.Obj) or (
                self.Obj['meta_data_dialog']['session_metadata']['LickSpoutReferenceArea'] == '') or (
                self.Obj['meta_data_dialog']['session_metadata']['LickSpoutReferenceX'] == '') or (
                self.Obj['meta_data_dialog']['session_metadata']['LickSpoutReferenceY'] == '') or (
                self.Obj['meta_data_dialog']['session_metadata']['LickSpoutReferenceZ'] == ''):
        ...
        self._initialize_fields(dic=self.Obj, keys=['fiber_photometry_start_time', 'fiber_photometry_end_time'],
                                default_value='')
        ...
        self._initialize_fields(dic=self.Obj['meta_data_dialog']['session_metadata'], keys=['ProjectName'],
                                default_value='')
        ...
        if 'B_AnimalResponseHistory' not in self.Obj:
            ...
            self.trials_rewarded = np.count_nonzero(
                np.logical_or(self.Obj['B_RewardedHistory'][0], self.Obj['B_RewardedHistory'][1]))
    ...
    def _combine_data_streams(self):
        ...
        if self.data_streams == []:
            ...
            self.data_streams[0].stream_modalities = self.data_streams[0].stream_modalities + self.ophys_streams[
                0].stream_modalities
            self.data_streams[0].stream_start_time = min(self.data_streams[0].stream_start_time,
                                                         self.ophys_streams[0].stream_start_time)
            self.data_streams[0].stream_end_time = max(self.data_streams[0].stream_end_time,
            ...
            self.data_streams[0].light_sources = self.data_streams[0].light_sources + self.ophys_streams[
                0].light_sources
            self.data_streams[0].detectors = self.data_streams[0].detectors + self.ophys_streams[0].detectors
            self.data_streams[0].fiber_connections = self.data_streams[0].fiber_connections + self.ophys_streams[
                0].fiber_connections
        ...
        if self.data_streams == []:
            ...
            self.data_streams[0].stream_modalities = self.data_streams[0].stream_modalities + self.ephys_streams[
                0].stream_modalities
            self.data_streams[0].stream_start_time = min(self.data_streams[0].stream_start_time,
            ...
            self.data_streams[0].ephys_modules = self.data_streams[0].ephys_modules + self.ephys_streams[
                0].ephys_modules
            self.data_streams[0].stick_microscopes = self.data_streams[0].stick_microscopes + self.ephys_streams[
                0].stick_microscopes
        ...
        if len(self.ephys_streams) > 2:
            ...
            for ephys_stream in self.ephys_streams[2:]:
                self.data_streams2.stream_start_time = min(self.data_streams2.stream_start_time,
                                                           ephys_stream.stream_start_time)
                self.data_streams2.stream_end_time = max(self.data_streams2.stream_end_time,
    ...
    def _get_high_speed_camera_stream(self):
        ...
        if self.Obj['Camera_dialog']['camera_start_time'] != '' and self.camera_names != []:
            if self.Obj['Camera_dialog']['camera_stop_time'] == '':
                ...
                camera_stop_time = datetime.strptime(self.Obj['Camera_dialog']['camera_stop_time'],
                                                     '%Y-%m-%d %H:%M:%S.%f')
            self.high_speed_camera_streams.append(Stream(
                stream_modalities=[Modality.BEHAVIOR_VIDEOS],
                camera_names=self.camera_names,
                stream_start_time=datetime.strptime(self.Obj['Camera_dialog']['camera_start_time'],
                                                    '%Y-%m-%d %H:%M:%S.%f'),
    ...
    def _get_photometry_detectors(self):
        ...
        '''
        self.fib_detectors = []
        exposure_time = datetime.strptime(self.Obj['fiber_photometry_end_time'],
    ...
    def _get_reward_probability(self):
        ...
        if self.session_model.experiment in ['Uncoupled Baiting', 'Uncoupled Without Baiting']:
            ...
            RewardProb = np.array(RewardPairs) / np.expand_dims(np.sum(RewardPairs, axis=1),
    ...
    def _get_stick_microscope(self):
        ...
        for stick_microscope in self.stick_microscope_names:
            self.stick_microscopes.append(DomeModule(
                ...
                rotation_angle=self.Obj['meta_data_dialog']['session_metadata']['microscopes'][stick_microscope][
                    'Stick_RotationAngle'],
                arc_angle=self.Obj['meta_data_dialog']['session_metadata']['microscopes'][stick_microscope][
                    'Stick_ArcAngle'],
                module_angle=self.Obj['meta_data_dialog']['session_metadata']['microscopes'][stick_microscope][
                    'Stick_ModuleAngle'],
        ...
        for ind_probe, probe in enumerate(self.probe_names):
            if probe in self.Obj['meta_data_dialog']['session_metadata']['probes']:
                self.ephys_modules.append(ManipulatorModule(
                    ...
                    primary_targeted_structure=self.Obj['meta_data_dialog']['session_metadata']['probes'][probe][
                        'ProbeTarget'],
                        ...
                        if Protocol == 'Sine':
                            for Frequency in RecentLaserCalibration[color][Protocol]:
                                ...
                                if f"Laser_{laser_tag}" in RecentLaserCalibration[color][Protocol][Frequency]:
                                    current_calibration = \
                                    RecentLaserCalibration[color][Protocol][Frequency][f"Laser_{laser_tag}"][
                                        'LaserPowerVoltage']
                                else:
                                    if laser_tag == 1:
                                        current_calibration = \
                                        RecentLaserCalibration[color][Protocol][Frequency]["Left"]['LaserPowerVoltage']
                                    elif laser_tag == 2:
                                        current_calibration = \
                                        RecentLaserCalibration[color][Protocol][Frequency]["Right"]['LaserPowerVoltage']
                                ...
                                self.parsed_optocalibration.append(
                                    {'laser name': laser, 'latest_calibration_date': latest_calibration_date,
                                     'Color': color, 'Protocol': Protocol, 'Frequency': Frequency,
                                     'Laser tag': laser_tag, 'Voltage': voltage, 'Power': power})
                        elif Protocol == 'Constant':
                            voltage = []
                            power = []
                            if f"Laser_{laser_tag}" in RecentLaserCalibration[color][Protocol]:
                                current_calibration = RecentLaserCalibration[color][Protocol][f"Laser_{laser_tag}"][
                                    'LaserPowerVoltage']
                            else:
                                if laser_tag == 1:
                                    current_calibration = RecentLaserCalibration[color][Protocol]["Left"][
                                        'LaserPowerVoltage']
                                elif laser_tag == 2:
                                    current_calibration = RecentLaserCalibration[color][Protocol]["Right"][
                                        'LaserPowerVoltage']
                            if current_calibration == []:
                                continue
                            for i in range(len(current_calibration)):
                            ...
                            self.parsed_optocalibration.append(
                                {'laser name': laser, 'latest_calibration_date': latest_calibration_date,
                                 'Color': color, 'Protocol': Protocol, 'Frequency': 'None', 'Laser tag': laser_tag,
    ...
    def _parse_water_calibration(self):
        ...
        for side in sides:
            ...
            sorted_X, sorted_Y = PlotWaterCalibration._GetWaterCalibration(self, self.WaterCalibrationResults,
                                                                           self.RecentWaterCalibrationDate, side)
    ...
    def _get_reward_delivery(self):
        ...
        if 'B_StagePositions' in self.Obj.keys():
            ...
            reference_spout_position = [float(self.Obj['meta_data_dialog']['session_metadata']['LickSpoutReferenceX']),
                                        float(
                                            self.Obj['meta_data_dialog']['session_metadata'].get('LickSpoutReferenceY',
                                                                                                 None)),
                                        float(
                                            self.Obj['meta_data_dialog']['session_metadata'].get('LickSpoutReferenceY1',
                                                                                                 None)),
                                        float(
                                            self.Obj['meta_data_dialog']['session_metadata'].get('LickSpoutReferenceY2',
                                                                                                 None)),
                                        float(self.Obj['meta_data_dialog']['session_metadata']['LickSpoutReferenceZ'])]
            reference_spout_position = [pos for pos in reference_spout_position if
                                        pos is not None]  # filter out None value
        ...
        if len(reference_spout_position) == 3:  # newscale stage
            ...
            right_lick_spout_reference_position = left_lick_spout_reference_position + np.array(
                [-lick_spouts_distance, 0, 0])
        else:  # aind stage
