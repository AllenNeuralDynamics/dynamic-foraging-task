...
class SchemaWidgetBase(QMainWindow):
    ...
    def __init__(self, schema: BaseModel):
        ...
        )  # Trigger update when property value changes
    ...
    def create_field_widgets(self, fields: dict, widget_field: str) -> dict:
        ...
        for name, value in fields.items():
            ...
            search_name = (
                arg_type.__name__
                if arg_type.__name__ in dir(self.schema_module)
                else name_lst[-1]
            )
            boxes = (
                {"label": QLabel(label_maker(name_lst[-1]))}
                if not name_lst[-1].isdigit()
                else {}
            )
            if (
                dict not in type(value).__mro__
                and list not in type(value).__mro__
                or type(arg_type) == enum.EnumMeta
            ):
            ...
            if (
                "." in name
            ):  # see if parent list and format index label and input vertically
    ...
    def create_attribute_widget(
        self, name, widget_type: Literal["combo", "text", "check"], values
    ):
    ...
    def create_combo_box(self, name: str, items: dict or list):
        ...
        box.currentTextChanged.connect(
            lambda value: self.combo_box_changed(value, name)
        )
    ...
    def combo_box_changed(self, value, name):
        ...
        value = (
            value_type[value]
            if type(value_type) == enum.EnumMeta
    ...
    def __setattr__(self, name, value):
        ...
        if (
            currentframe().f_back.f_locals.get("self", None) != self
...
def label_maker(string):
    ...
    for i, word in enumerate(label):
        for unit in possible_units:
            if (
                unit.lower() == word.lower()
            ):  # TODO: Consider using regular expression here for better results?
                label[i] = f"[{unit}]"
...
def add_border(
    ...
    if len(widgets) % 2 != 0 and orientation in [
        "VH",
        "HV",
    ]:  # add dummy widget so all rows/colums can be created
