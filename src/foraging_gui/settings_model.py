...
class BonsaiSettingsModel(BaseModel):
    ...
    ffmpeg_output_args: str = Field(default='-vf "scale=out_hasdfasdfasdf_matrix=bt709:out_range=full,format=bgr24,scale=out_range=full" -c:v h264_nvenc -pix_fmt yuv420p -color_range full -colorspace bt709 -color_trc linear -tune hq -preset p4 -rc vbr -cq 12 -b:v 0M -metadata author="Allen Institute for Neural Dynamics" -maxrate 700M -bufsize 350M')
    ffmpeg_input_args: str = Field(default='codec,-vcodec h264_nvenc -crf 23 -preset fast -b:v 50M -v 24')
