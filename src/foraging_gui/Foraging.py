...
class Window(QMainWindow):
    ...
    def load_curriculum(self, mouse_id: str) -> None:
        ...
        try:
            ...
            trainer_state, slims_session, task, sess, opto, fip, oc = (
                self.slims_handler.load_mouse_curriculum(mouse_id)
            )
            ...
            logging.info(
                f"Successfully loaded mouse {mouse_id}",
                extra={"tags": [self.warning_log_tag]},
            )
    ...
    def create_curriculum(self, mouse_id) -> tuple[
        ...
        if not (
            ...
            self.fip_model = FiberPhotometry(
                mode=mode,
                stage_start=(
                    "stage_1_warmup" if type(first) != str else first.lower()
                ),
            )
            # check if current stage is past stage_start and enable if so
            self.fip_model.enabled = stage_list.index(
                ts.stage.name
    ...
    def write_curriculum(self):
        ...
        if (
            ...
            and self.slims_handler.curriculum is not None
        ):
            try:
    ...
    def _GetSaveFolder(self):
        ...
        if self.load_tag == 0:
            ...
            self.session_model.session_name = f"behavior_{self.session_model.subject}_{self.session_model.date.strftime('%Y-%m-%d_%H-%M-%S')}"
        else:
    ...
    def update_model_widgets(self):
        ...
        self.on_curriculum.setVisible(
            self.slims_handler.loaded_mouse_id is not None
        )
