...
from PyQt5.QtCore import (QAbstractTableModel, QItemSelectionModel, QObject,
                          Qt, QThreadPool, QTimer)
    ...
    def _Frequency(self, Numb):
        try:
            ...
            if latest_calibration_date == "NA":
                ...
                RecentLaserCalibration = self.MainWindow.LaserCalibrationResults[
            ...
            for laser_tag in self.laser_tags:
                ...
                if Protocol in ["Sine"]:
                    for i in range(
                        len(
                            RecentLaserCalibration[Color][Protocol][CurrentFrequency][
                        ...
                        ItemsLaserPower.append(
                            str(
                                RecentLaserCalibration[Color][Protocol][
                                    CurrentFrequency
                                ][f"Laser_{str(laser_tag)}"]["LaserPowerVoltage"][i]
                            )
                        )
                ...
                getattr(self, f"Laser{str(laser_tag)}_power_{str(Numb)}").addItems(
                    ItemsLaserPower
                )
                index = getattr(
                    self, f"Laser{str(laser_tag)}_power_{str(Numb)}"
                            ...
                            for laser_tag in self.laser_tags:
                                ...
                                for i in range(
                                    len(
                                        RecentLaserCalibration[Color][Protocol][
                                            CurrentFrequency
                                        ][f"Laser_{laser_tag}"]["LaserPowerVoltage"]
                                    )
                                ):
                                    ItemsLaserPower.append(
                                        str(
...
class AutoTrainDialog(QDialog):
    ...
    def _setup_allbacks(self):
        ...
        self.checkBox_override_stage.stateChanged.connect(self._override_stage_clicked)
        ...
        self.pushButton_apply_curriculum.clicked.connect(self._apply_curriculum)
    ...
    def _preview_auto_train_paras(self, preview_checked):
        ...
        if preview_checked:
            ...
            paras = self.curriculum_in_use.parameters[TrainingStage[self.stage_in_use]]
    ...
    def _set_training_parameters(self, paras_dict, if_apply_and_lock=False):
        ...
        if "warmup" in keys:
            ...
            if (
                paras_dict
                and paras_dict["warmup"] != self.MainWindow.warmup.currentText()
            ):
