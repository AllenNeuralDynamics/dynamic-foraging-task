...
class WaterCalibrationDialog(QDialog):
    ...
    def _StartCalibratingLeft(self):
        ...
        self.left_opentimes = np.arange(
            float(self.params["TimeMin"]), float(self.params["TimeMax"]) + 0.0001, float(self.params["Stride"])
        )
    ...
    def _CalibrateLeftOne(self, repeat=False):
        ...
        before_weight, ok = QInputDialog().getDouble(
            self, "Box {}, Left".format(self.MainWindow.box_letter), "Before weight (g): ", before_weight, 0, 1000, 4
        )
        if not ok:
            # User cancels
        ...
        final_tube_weight, ok = QInputDialog().getDouble(
            self, "Box {}, Left".format(self.MainWindow.box_letter), "Weight after (g): ", final_tube_weight, 0, 1000, 4
        )
        if not ok:
            self.Warning.setText("Please repeat measurement")
            self.WeightBeforeLeft.setText("")
    ...
    def _CalibrateRightOne(self, repeat=False):
        ...
        if self.WeightBeforeRight.text() != "":
            ...
            before_weight = 0.0
        before_weight, ok = QInputDialog().getDouble(
            self, "Box {}, Right".format(self.MainWindow.box_letter), "Before weight (g): ", before_weight, 0, 1000, 4
        )
        if not ok:
            # User cancels
            self.Warning.setText("Press Continue, Repeat, or Finished")
    ...
    def _SpotCheck(self, valve: Literal["Left", "Right"]):
        ...
        if total_water.text() != "":
            ...
            final_tube_weight = 0.0
        final_tube_weight, ok = QInputDialog().getDouble(
            self, f"Box {self.MainWindow.box_letter}, {valve}", "Final tube weight (g): ", final_tube_weight, 0, 1000, 4
        )
            ...
            try:
                subprocess.Popen(
                    [
                        "explorer",
                        os.path.join(
                            os.path.dirname(os.path.dirname(self.MainWindow.Ot_log_folder)), "behavior-videos"
                        ),
                    ]
                )
                ...
                for k in range(len(Protocols_unique)):
                    ...
                    if current_protocol == "Sine":
                        ...
                        for laser_tag in self.laser_tags:
                            ...
                            LaserCalibrationResults = initialize_dic(
                                LaserCalibrationResults,
                                key_list=[current_date_name, current_color, current_protocol, f"Laser_{laser_tag}"],
                            )
                            if (
                                "LaserPowerVoltage"
                                not in LaserCalibrationResults[current_date_name][current_color][current_protocol][
                            ...
                            if current_protocol == "Constant":  # copy results of constant to pulse
                                LaserCalibrationResults = initialize_dic(
                                    LaserCalibrationResults,
                                    key_list=[current_date_name, current_color, "Pulse", f"Laser_{laser_tag}"],
                                )
                                if (
                                    "LaserPowerVoltage"
...
class MetadataDialog(QDialog):
    ...
    def _load_metadata(self):
        ...
        metadata_dialog_file, _ = QFileDialog.getOpenFileName(
            self, "Select Metadata File", self.MainWindow.metadata_dialog_folder, "JSON Files (*.json)"
        )
    ...
    def _save_configuration(self):
        ...
        for i in range(len(probe_types)):
            ...
            self.meta_data["session_metadata"] = initialize_dic(
                self.meta_data["session_metadata"], key_list=[metadata_key, current_probe]
            )
    ...
    def _show_stick_microscopes(self):
        ...
        self._manage_signals(
            enable=False, keys=self._get_children_keys(self.Microscopes), action=self._save_configuration
        )
        self.StickMicroscopes.clear()
        self.StickMicroscopes.addItems(items)
        self._manage_signals(enable=True, keys=["StickMicroscopes"], action=self._show_angles)
        self._manage_signals(
            enable=True, keys=self._get_children_keys(self.Microscopes), action=self._save_configuration
        )
        self._show_angles()
    ...
    def _SelectRigMetadata(self, rig_metadata_file=None):
        ...
        if rig_metadata_file is None:
            rig_metadata_file, _ = QFileDialog.getOpenFileName(
                self, "Select Rig Metadata File", self.MainWindow.rig_metadata_folder, "JSON Files (*.json)"
            )
