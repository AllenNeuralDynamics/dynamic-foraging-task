...
if __name__ == "__main__":
    ...
    warning_timer = QTimer(
        timeout=lambda: logger.warning(warnings[randint(0, 5)], extra={"tags": "warning_widget"}), interval=1000
    )

    info_timer = QTimer(
        timeout=lambda: logger.info(infos[randint(0, 2)], extra={"tags": "warning_widget"}), interval=1500
    )
    error_timer = QTimer(
        timeout=lambda: logger.error(errors[randint(0, 2)], extra={"tags": "warning_widget"}), interval=1750
    )
