...
def task_parameters_to_tp_conversion(task_parameters: AindDynamicForagingTaskParameters) -> dict:
    ...
    return {
        ...
        "TP_warm_max_choice_ratio_bias": None
        ...
        "warm_max_choice_ratio_bias": None
        if task_parameters.warmup is None
        else task_parameters.warmup.max_choice_ratio_bias,
    ...
    return AindDynamicForagingTaskLogic(
        task_parameters=AindDynamicForagingTaskParameters(
            ...
            inter_trial_interval=InterTrialInterval(
                ...
                beta=0 if behavior["TP_ITIBeta"] == "" else float(behavior["TP_ITIBeta"]),
                increase=0 if behavior["TP_ITIIncrease"] == "" else float(behavior["TP_ITIIncrease"]),
            ),
            response_time=ResponseTime(
                response_time=0 if behavior["TP_ResponseTime"] == "" else float(behavior["TP_ResponseTime"]),
                reward_consume_time=0
...
def behavior_json_to_fip_model(behavior_json: dict) -> FiberPhotometry:
    ...
    return FiberPhotometry(
        enabled=behavior["TP_PhotometryB"], mode=behavior["TP_FIPMode"], baseline_time=behavior["TP_baselinetime"]
    )
...
def behavior_json_to_opto_model(behavior_json: dict) -> Optogenetics:
    ...
    sort_map = [LaserColorOne, LaserColorTwo, LaserColorThree, LaserColorFour, LaserColorFive, LaserColorSix]
        ...
        if behavior[f"TP_LaserColor_{i + 1}"] != "NA":
            ...
            if behavior[f"TP_Protocol_{i + 1}"] == "Sine":
                protocol = SineProtocol(
                    frequency=behavior[f"TP_Frequency_{i + 1}"], ramp_down=behavior[f"TP_RD_{i + 1}"]
                )
            elif behavior[f"TP_Protocol_{i + 1}"] == "Pulse":
                protocol = PulseProtocol(
                    frequency=behavior[f"TP_Frequency_{i + 1}"], duration=behavior[f"TP_PulseDur_{i + 1}"]
                )
            else:
            ...
            lasers.append(
                sort_map[i](
                    ...
                    start=IntervalConditions(
                        ...
                        interval_condition=behavior[f"TP_LaserEnd_{i + 1}"], offset=behavior[f"TP_OffsetEnd_{i + 1}"]
                    ),
                    protocol=protocol,
                )
            )
