...
def task_parameters_to_tp_conversion(
    ...
    return {
        ...
        "AdvancedBlockAuto": (
            task_parameters.auto_block
            if task_parameters.auto_block is not None
            else "off"
        ),
        "AutoReward": (
        ...
        "AutoWaterType": (
            ...
            else task_parameters.auto_water.auto_water_type
        ),
        ...
        "Ignored": (
            None
            if task_parameters.auto_water is None
            else task_parameters.auto_water.ignored
        ),
        "IncludeAutoReward": (
            None
            if task_parameters.auto_water is None
            else task_parameters.auto_water.include_reward
        ),
        "InitiallyInactiveN": (
            None
            if task_parameters.reward_n is None
            else task_parameters.reward_n.initial_inactive_trials
        ),
        "LeftValue_volume": task_parameters.reward_size.left_value_volume,
        "Multiplier": (
            None
            if task_parameters.auto_water is None
            else task_parameters.auto_water.multiplier
        ),
        "PointsInARow": (
            None
            if task_parameters.auto_block is None
            else task_parameters.auto_block.points_in_a_row
        ),
        ...
        "SwitchThr": (
            None
            if task_parameters.auto_block is None
            else task_parameters.auto_block.switch_thr
        ),
        ...
        "TP_IncludeAutoReward": (
            None
            if task_parameters.auto_water is None
            else task_parameters.auto_water.include_reward
        ),
        "TP_InitiallyInactiveN": (
            None
            if task_parameters.reward_n is None
            else task_parameters.reward_n.initial_inactive_trials
        ),
        ...
        "TP_Unrewarded": (
            None
            if task_parameters.auto_water is None
            else task_parameters.auto_water.unrewarded
        ),
        "TP_warm_max_choice_ratio_bias": (
            None
            if task_parameters.warmup is None
            else task_parameters.warmup.max_choice_ratio_bias
        ),
        "TP_warm_min_finish_ratio": (
            None
            if task_parameters.warmup is None
        ...
        "TP_warm_windowsize": (
            None
            if task_parameters.warmup is None
            else task_parameters.warmup.windowsize
        ),
...
def behavior_json_to_task_logic_model(
    behavior_json: dict,
) -> AindDynamicForagingTaskLogic:
    """
    ...
    return AindDynamicForagingTaskLogic(
        task_parameters=AindDynamicForagingTaskParameters(
            ...
            reward_probability=RewardProbability(
                base_reward_sum=(
                    0
                    if behavior["TP_BaseRewardSum"]
                    else float(behavior["TP_BaseRewardSum"])
                ),
                family=(
                    0
                    if behavior["TP_RewardFamily"]
                    else float(behavior["TP_RewardFamily"])
                ),
                pairs_n=(
                    0
                    if behavior["TP_RewardPairsN"]
                    else float(behavior["TP_RewardPairsN"])
                ),
            ),
            uncoupled_reward=(
                None
                if behavior["TP_UncoupledReward"][-1] == ""
                else list(ast.literal_eval(behavior["TP_UncoupledReward"]))
            ),
            ...
            reward_delay=(
                0
                if behavior["TP_RewardDelay"] == ""
                else float(behavior["TP_RewardDelay"])
            ),
            auto_water=(
                None
                if not behavior["TP_AutoReward"]
                else AutoWater(
                    auto_water_type=behavior["TP_AutoWaterType"],
                    multiplier=(
                        0
                        if behavior["TP_Multiplier"] == ""
                        else float(behavior["TP_Multiplier"])
                    ),
                    unrewarded=(
                        0
                        if behavior["TP_Unrewarded"] == ""
                        else float(behavior["TP_Unrewarded"])
                    ),
                    ignored=(
                        0
                        if behavior["TP_Ignored"] == ""
                        else float(behavior["TP_Ignored"])
                    ),
                    include_reward=(
                        0
                        if behavior["TP_IncludeAutoReward"] == ""
                        else float(behavior["TP_IncludeAutoReward"])
                    ),
                )
            ),
            inter_trial_interval=InterTrialInterval(
                ...
                increase=(
                    0
                    if behavior["TP_ITIIncrease"] == ""
                    else float(behavior["TP_ITIIncrease"])
                ),
            ),
            response_time=ResponseTime(
                response_time=(
                    0
                    if behavior["TP_ResponseTime"] == ""
                    else float(behavior["TP_ResponseTime"])
                ),
                reward_consume_time=(
                    0
                    if behavior["TP_RewardConsumeTime"] == ""
                    else float(behavior["TP_RewardConsumeTime"])
                ),
            ),
            auto_block=(
                None
                if behavior["TP_AdvancedBlockAuto"]
                else AutoBlock(
                    advanced_block_auto=behavior["TP_AdvancedBlockAuto"],
                    switch_thr=(
                        0
                        if behavior["TP_SwitchThr"] == ""
                        else float(behavior["TP_SwitchThr"])
                    ),
                    points_in_a_row=(
                        0
                        if behavior["TP_PointsInARow"] == ""
                        else float(behavior["TP_PointsInARow"])
                    ),
                )
            ),
            warmup=(
                None
                if not behavior["TP_warmup"]
                else Warmup(
                    min_trial=(
                        0
                        if behavior["TP_warm_min_trial"] == ""
                        else float(behavior["TP_warm_min_trial"])
                    ),
                    max_choice_ratio_bias=(
                        0
                        if behavior["TP_warm_max_choice_ratio_bias"] == ""
                        else float(behavior["TP_warm_max_choice_ratio_bias"])
                    ),
                    min_finish_ratio=(
                        0
                        if behavior["TP_warm_min_finish_ratio"] == ""
                        else float(behavior["TP_warm_min_finish_ratio"])
                    ),
                    windowsize=(
                        0
                        if behavior["TP_warm_windowsize"] == ""
                        else float(behavior["TP_warm_windowsize"])
                    ),
                )
            ),
            reward_size=RewardSize(
                right_value_volume=(
                    0
                    if behavior["TP_RightValue_volume"] == ""
                    else float(behavior["TP_RightValue_volume"])
                ),
                left_value_volume=(
                    0
                    if behavior["TP_LeftValue_volume"] == ""
                    else float(behavior["TP_LeftValue_volume"])
                ),
            ),
            ...
            reward_n=(
                None
                if behavior["TP_Task"] != "RewardN"
                else RewardN(
                    initial_inactive_trials=(
                        0
                        if behavior["TP_InitiallyInactiveN"] == ""
                        else float(behavior["TP_InitiallyInactiveN"])
                    )
                )
            ),
        )
...
def behavior_json_to_session_model(
    ...
    return AindBehaviorSessionModel(
        ...
        date=datetime.strptime(
            behavior_json["Other_SessionStartTime"], "%Y-%m-%d %H:%M:%S.%f"
        ),
...
def opto_to_tp_conversion(opto_model: Optogenetics) -> dict:
    ...
    dictionary = {
        "TP_FractionOfSession": getattr(
            opto_model.session_control, "session_fraction", None
        ),
        "TP_MinOptoInterval": opto_model.minimum_trial_interval,
        "TP_SampleFrequency": opto_model.sample_frequency,
        "TP_SessionAlternating": getattr(
            opto_model.session_control, "alternating_sessions", None
        ),
    ...
    for i in range(0, 6):
        ...
        if lasers[laser_name] is not None:
            dictionary[f"TP_ConditionP_{i + 1}"] = lasers[
                laser_name
            ].condition_probability
            dictionary[f"TP_Condition_{i + 1}"] = getattr(
                lasers[laser_name], "pulse_condition", None
            )
            dictionary[f"TP_Duration_{i + 1}"] = lasers[laser_name].duration
            dictionary[f"TP_Frequency_{i + 1}"] = getattr(
                lasers[laser_name].protocol, "frequency", None
            )
            ...
            dictionary[f"TP_RD_{i + 1}"] = getattr(
                lasers[laser_name].protocol, "ramp_down", None
            )
...
def behavior_json_to_opto_model(behavior_json: dict) -> Optogenetics:
    ...
    return Optogenetics(
        ...
        session_control=(
            None
            if not behavior["TP_SessionWideControl"]
            else SessionControl(
...
def operational_control_to_tp_conversion(
...
def behavior_json_to_operational_control_model(
    behavior_json: dict,
) -> OperationalControl:
    """
    ...
    return OperationalControl(
        ...
        stage_specs=(
            None
            if behavior["B_StagePositions"] is None
            else StageSpecs(
                stage_name=(
                    "newscale"
                ...
                y=behavior["B_StagePositions"]["y"]
                or behavior["B_StagePositions"]["y1"],
