...
class BiasIndicator(QMainWindow):
    ...
    def __init__(self, bias_threshold: float = 0.7, x_range: int = 15, *args, **kwargs):
        """
        ...
        self.bias_plot.setRange(
            xRange=[1, self.x_range], yRange=[-self.bias_threshold - 0.3, 0.3 + self.bias_threshold]
        )
        ...
        self.bias_plot.addItem(
            FillBetweenItem(
                curve1=self._upper_scatter_item, curve2=self._lower_scatter_item, pen="lightgray", brush="lightgray"
            )
        )
    ...
    def calculate_bias(
        ...
        if len(choice_history[~np.isnan(choice_history)]) >= n_trial_back + 2:
            ...
            self._upper_scatter_item.setData(
                x=np.append(self._upper_scatter_item.xData, trial_num),
                y=np.append(self._upper_scatter_item.yData, upper),
            )
            self._lower_scatter_item.setData(
                x=np.append(self._lower_scatter_item.xData, trial_num),
            ...
            if len(self._biases) >= 2:
                ...
                if trial_num >= self.bias_plot.getAxis("bottom").range[1] - 50:
                    self.bias_plot.setRange(
                        xRange=[trial_num - self.x_range if self.x_range < trial_num else 2, trial_num + 50]
                    )
            ...
            if abs(bias) > self.bias_threshold:
                ...
                self._current_bias_point.setData(
                    pos=[[trial_num, bias]], pen=QColor("purple"), brush=QColor("purple"), size=9
                )

            else:
                self._current_bias_point.setData(
                    pos=[[trial_num, bias]], pen=QColor("green"), brush=QColor("green"), size=9
                )
