...
class GenerateTrials:
    ...
    def _GetBasic(self):
        ...
        BS_auto_water_left, BS_earned_reward_left, BS_AutoWater_N_left, BS_EarnedReward_N_left = self._process_values(
            self.Obj["TP_LeftValue_volume"], self.B_AutoWaterTrial[0], self.Obj["TP_Multiplier"], B_RewardedHistory[0]
        )
    ...
    def _CheckStop(self):
        ...
        if (
            ...
            and len(np.where(non_auto_reward[-win_sz:] == 2)[0]) >= StopIgnore
        ):
            stop = True
            ...
            warning_label_text = (
                "Stop because ignore trials exceed or equal: " + f"{threshold}% of {self.TP_auto_stop_ignore_win}"
            )
    ...
    def _CheckAutoWater(self):
        ...
        if self.TP_AutoReward:
            ...
            if UnrewardedN <= 0:
                ...
                logging.warning(
                    "Auto water because unrewarded trials exceed: " + self.TP_Unrewarded,
                    extra={"tags": [self.win.warning_log_tag]},
                )
            elif IgnoredN <= 0:
                logging.warning(
    ...
    def _GetLaserWaveForm(self):
        ...
        if (
            ...
            or self.CLP_LaserStart == "Go cue"
            or self.CLP_LaserStart == "Reward outcome"
    ...
    def _GetLaserAmplitude(self):
        ...
        if self.CLP_Location == "Laser_1":
            ...
            if self.CLP_Laser1Power == "" or self.CLP_Location == "":
                logging.warning(
                    "No amplitude for Laser_1 or Laser_2 laser defined!", extra={"tags": [self.win.warning_log_tag]}
                )
            else:
                Laser1PowerAmp = eval(self.CLP_Laser1Power)
                Laser2PowerAmp = eval(self.CLP_Laser2Power)
    ...
    def _SimulateResponse(self):
        ...
        if TN == 0:
            ...
            TrialStartTimeHarp = (
                ...
                + float(self.Obj["TP_RewardConsumeTime"][TN - 1])
            )
        ...
        TrialEndTimeHarp = (
            GoCueTimeBehaviorBoard + self.B_ResponseTimeHistory[TN] + float(self.Obj["TP_RewardConsumeTime"][TN])
        )
